...
struct EulerEquations
{
   ...
   static
   typename InputVector::value_type
   compute_mach (const InputVector &W)
   {
      typedef typename InputVector::value_type number;
      //compute velocity
      number velocity = 0;
      for (unsigned int d=0; d<dim; ++d)
         velocity += *(W.begin()+d) * *(W.begin()+d);  
      velocity = std::sqrt(velocity) / (*(W.begin()+density_component));
      //compute sound speed
      const number pressure = compute_pressure<number> (W);
      const number sonic = std::sqrt(gas_gamma * pressure / (*(W.begin()+density_component)));
   ...
   //---------------------------------------------------------------------------
   // compute the Jacobi matrix of a given W vector
   // Note: This is implemented only for 2-D
   //---------------------------------------------------------------------------
   template <typename InputVector>
   static
   void compute_Jacobi_matrix (const InputVector &W,
                               //number (&A_x)[n_components][n_components],
                               //number (&A_y)[n_components][n_components]
                   std::array<std::array<typename InputVector::value_type, n_components>, n_components> &A_x,
                   std::array<std::array<typename InputVector::value_type, n_components>, n_components> &A_y
                               )
   {
     typedef typename InputVector::value_type number;
     number g1   = gas_gamma - 1.0;
     number rho  = W[density_component];
     number E    = W[energy_component];   //E=rho*e_total
     number u    = W[0] / rho;
     number v    = W[1] / rho;

     number q2   = u*u + v*v;
     number p    = g1 * (E - 0.5 * rho * q2);  
     number c2   = gas_gamma * p / rho;  //square of speed of sound
     //double c    = std::sqrt(c2);        //speed of sound
     number phi2 = 0.5*g1*q2;
     number h    = c2/g1 + 0.5*q2;     //h=e_in+p/rho+0.5*q2

     A_x[0][0]=(3-gas_gamma)*u; A_x[0][1]=-g1*v;   A_x[0][2]=-u*u+phi2;   A_x[0][3]=g1;
     A_x[1][0]=v;               A_x[1][1]=u;       A_x[1][2]=-u*v;        A_x[1][3]=0;
     A_x[2][0]=1;               A_x[2][1]=0;       A_x[2][2]=0;           A_x[2][3]=0;
     A_x[3][0]=h-g1*u*u;        A_x[3][1]=-g1*u*v; A_x[3][2]=u*(phi2-h);  A_x[3][3]=gas_gamma*u; 

     A_y[0][0]=v;       A_y[0][1]=u;               A_y[0][2]=-u*v;        A_y[0][3]=0;
     A_y[1][0]=-g1*u;   A_y[1][1]=(3-gas_gamma)*v; A_y[1][2]=-v*v+phi2;   A_y[1][3]=g1;
     A_y[2][0]=0;       A_y[2][1]=1;               A_y[2][2]=0;           A_y[2][3]=0;
     A_y[3][0]=-g1*u*v; A_y[3][1]=h-g1*v*v;        A_y[3][2]=v*(phi2-h);  A_y[3][3]=gas_gamma*v;  
   }
