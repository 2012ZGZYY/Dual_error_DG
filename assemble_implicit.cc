                                                ...
                                                CellInfo& info)
{
   ...
   const unsigned int n_components  = EulerEquations<dim>::n_components; //new added variable
   // Get time step for current cell
   // I add the sentence "if...". otherwise it will cause error when parameters.is_stationary == true, since in this case the Vector "dt" has not been initialized, its size is 0. so the call "dt(cell_number)" is illegal.
   double tmp_dt;
   if(parameters.is_stationary == false)
      tmp_dt = dt( cell_number(dinfo.cell) );
   else
      tmp_dt = 1e20; 
   const double time_step = tmp_dt;

   Table<2,Sacado::Fad::DFad<double> >
      dWdt (n_q_points, n_components);
   
   Table<2,Sacado::Fad::DFad<double> >
      W_theta (n_q_points, n_components);
   
   Table<3,Sacado::Fad::DFad<double> >
      grad_W (n_q_points, n_components, dim);
   ...
   // computed from it:
   //typedef Sacado::Fad::DFad<double> FluxMatrix[n_components][dim];
   //FluxMatrix *flux = new FluxMatrix[n_q_points];
   std::vector<std::array<std::array<Sacado::Fad::DFad<double>, dim>,
                          n_components>>   flux(n_q_points);

   //typedef Sacado::Fad::DFad<double> ForcingVector[n_components];
   //ForcingVector *forcing = new ForcingVector[n_q_points];
   std::vector<std::array<Sacado::Fad::DFad<double>, n_components>>    forcing(n_q_points);
